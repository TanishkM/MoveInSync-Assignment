...
const addUser = async (req, res) => {
  ...
  try {
    if((role.localeCompare('traveler')!= 0)  && (role.localeCompare('traveler-companion')!=0)){
    ...
    const isTravelerC = role.localeCompare('traveler-companion')==0
    
    const newUser = new User({email, password: hashedPassword,isTraveler,isTravelerC,username,phoneNo});
    ...
    const jsonPayload = JSON.stringify({ email, otp });
    const hash = crypto.createHash('sha256').update(jsonPayload).digest('hex');
    const verificationData = new Verification({ email, hash });
    ...
    try {
      ...
      res.status(500).json({ message: 'Email not sent. Please contact the developers.', success: false, data: null });
    }
    
    res.status(201).json({ message: "User created successfully. Verification Link sent to your email.", success: true, data: null });
  } catch (error) {
...
const toggleTravelerC =  async (req, res) => {
  ...
  try {
    ...
    await User.updateOne({ email }, { isTravelerC: !user.isTravelerC });
...
module.exports = { addUser,verifyUser,loginUser ,addAdmin,forgetPassword,resetPasswordOtp,resetPassword,toggleTraveler,toggleTravelerC};
